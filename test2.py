  ...
  with TFile(Sample(productionmode, rwtfrom, production).withdiscriminantsfile()) if rwtfrom != "all" else DummyContextManager() as f:
    if rwtfrom == "all":
      ...
      for rwtfrom in "0+", "0-", "a2", "L1", "fa3prod0.5", "fa2prod0.5", "fL1prod0.5":
        tree.Add(Sample(productionmode, rwtfrom, production).withdiscriminantsfile())
    else:
      tree = f.candTree
    ...
    return getattr(ROOT, hname).Integral()

def allcats(**kwargs):
  counts = {_: count(category=_, **kwargs) for _ in categories}
  return counts
  for _ in categories: print "{:10} {:>7.3%}".format(_, counts[_] / sum(counts.values()))
