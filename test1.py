...
def f(analysis, production, channel, category, hypothesis, productionmode):
    from helperstuff.combinehelpers import gettemplate
    multiply = 1 if hypothesis == "0+" else gp4[analysis]
    print "  Heshy  ", gettemplate(analysis, production, channel, category, productionmode, hypothesis).Integral() * multiply
    print "  Ulascan", gettemplate(analysis, str(production)+"_Ulascan", channel, category, productionmode, hypothesis).Integral() * multiply
...
p.add_argument("--productionmode", choices=("VBF", "ZH", "WH", "ggH"), default="VBF")
args = p.parse_args()
